...
export type ThemeGame = "swsh" | "sv" | "bdsp";
...
const numberedWallpapers = (folder: string, prefix: string, count: number) =>
  Array.from({ length: count }, (_, index) => themeAssetUrl(`assets/themes/${folder}/${prefix}-${String(index + 1).padStart(2, "0")}.png`));
...
function sanitizeTheme(value: unknown): BoxTheme | null {
  ...
  if (candidate.kind === "preset" && (candidate.game === "swsh" || candidate.game === "sv" || candidate.game === "bdsp") && typeof candidate.wallpaper === "string" && allowedWallpapers.has(candidate.wallpaper)) return createPresetTheme(candidate.game, candidate.wallpaper);
